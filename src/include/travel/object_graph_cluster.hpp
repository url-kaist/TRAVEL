...
class ObjectCluster {
    ...
    public:
        ...
        void sphericalProjection(boost::shared_ptr<pcl::PointCloud<T>> cloud_in) {
            boost::shared_ptr<pcl::PointCloud<T>> valid_cloud = 
            ...
            for (size_t i = 0; i < valid_indices.size(); i++) {
                ...
                for (auto &p : clusters_[idx]) {
                    if (p->valid) {
                        ...
                        if (!DEBUG) { // release mode
                            T point = cloud_in->points[p->idx];
                            point.label = label;
                            cloud_out->points.push_back(point);
                        } else {  // debug mode
                            cloud_in->points[p->idx].id = label;
                        }
                        pt_cnt++;
                    } else {
        ...
        void horizontalUpdate(int channel) {
            ...
            //     for (auto n : nodes_[channel]){
            //         printf("label: %d, start: %d, end: %d\n", n.label, n.start, n.end);  
            //         for (auto el : clusters_[n.label]) {
            //             printf("%f;%f;%f\n", el->x, el->y, el->z);
            //         } 
            //     }
            // }
        ...
        void verticalUpdate(int channel) {
            ...
            for(int n = 0; n < nodes_[channel].size(); n++) {
                ...
                for (int l = prev_channel; l >= channel - VERT_SCAN_SIZE; l -= 1) {
                    ...
                    // if (DEBUG) {
                    //     printf("---cur line: %d, prev_channel: %d\n", channel, l);
