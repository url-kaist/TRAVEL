#ifndef TILEWORK_TRIGRID_H
#define TILEWORK_TRIGRID_H
...
public:
    ...
    TravelGroundSeg(ros::NodeHandle* nh):node_handle_(*nh){
        ...
        // node_handle_.param("travelgseg/max_r", MAX_RANGE_, 40.0);
        // ROS_INFO("Max Range: %f", MAX_RANGE_);

        // node_handle_.param("travelgseg/min_r", MIN_RANGE_, 1.0);
        // ROS_INFO("Min Range: %f", MIN_RANGE_);

        // node_handle_.param("travelgseg/resolution", TGF_RESOLUTION_, 5.0);
        // ROS_INFO("Resolution: %f", TGF_RESOLUTION_);

        // node_handle_.param("/travelgseg/num_iter", NUM_ITER_, 3);
        // ROS_INFO("Num of Iteration: %d", NUM_ITER_);

        // node_handle_.param("/travelgseg/num_lpr", NUM_LRP_, 20);
        // ROS_INFO("Num of LPR: %d", NUM_LRP_);

        // node_handle_.param("travelgseg/num_min_pts", NUM_MIN_POINTS_, 10);
        // ROS_INFO("Num of min. points: %d", NUM_MIN_POINTS_);

        // node_handle_.param("travelgseg/th_seeds", TH_SEEDS_, 0.4);
        // ROS_INFO("Seeds Threshold: %f", TH_SEEDS_);

        // node_handle_.param("travelgseg/th_dist", TH_DIST_, 0.3);
        // ROS_INFO("Distance Threshold: %f", TH_DIST_);

        // node_handle_.param("travelgseg/th_outlier", TH_OUTLIER_, 0.3);
        // ROS_INFO("Outlier Threshold: %f", TH_OUTLIER_);

        // node_handle_.param("travelgseg/normal_thr", TH_NORMAL_, 0.707);
        // ROS_INFO("Normal Threshold: %f", TH_NORMAL_);

        // node_handle_.param("travelgseg/planar_confidence_threshold", TH_WEIGHT_, 1.5);
        // ROS_INFO("Node Weight Threshold: %f", TH_WEIGHT_);

        // node_handle_.param("travelgseg/local_convex_normal", TH_LCC_NORMAL_SIMILARITY_ , 1.5);
        // ROS_INFO("LCC Normal Similarity: %f", TH_LCC_NORMAL_SIMILARITY_);

        // node_handle_.param("travelgseg/local_convex_planar", TH_LCC_PLANAR_MODEL_DIST_ , 1.5);
        // ROS_INFO("LCC Plane Distance   : %f", TH_LCC_PLANAR_MODEL_DIST_);

        // node_handle_.param("travelgseg/refine_mode", REFINE_MODE_, true);
        // node_handle_.param("travelgseg/visualization", VIZ_MDOE_, true);  
    ...
    void setParams(const double max_range, const double min_range, const double resolution, 
                        ...
                        const double th_lcc_normal_similiarity, const double th_lcc_planar_model_dist, 
    ...
    void estimateGround(const pcl::PointCloud<PointType>& cloud_in,
        ...
        segmentTGFGround(trigrid_field_, ptCloud_tgfwise_ground_, ptCloud_tgfwise_nonground_, ptCloud_tgfwise_outliers_);
...
private:
    ...
    void extractInitialSeeds(const pcl::PointCloud<PointType>& p_sorted, pcl::PointCloud<PointType>& init_seeds){
        ...
        for (int i=0; i<p_sorted.points.size() && cnt<NUM_LRP_; i++){
    ...
    void findDominantNode(const TriGridField<PointType>& tgf_in, TriGridIdx& node_idx_out) {
        ...
        TriGridIdx temp_idx;
        temp_idx.row = (int)((0-tgf_min_x)/TGF_RESOLUTION_);
        temp_idx.col = (int)((0-tgf_min_y)/TGF_RESOLUTION_);
        temp_idx.tri = 0;
        int max_tri = 0;
        for (int s_i = 0 ; s_i < tgf_in[temp_idx.row][temp_idx.row].size(); s_i++){
            if (tgf_in[temp_idx.row][temp_idx.row][s_i].weight > tgf_in[temp_idx.row][temp_idx.row][max_tri].weight)
                temp_idx.tri = s_i;
        }
        node_idx_out = temp_idx;
        return;
    }
    ...
    void searchNeighborNodes(const TriGridIdx &cur_idx, std::vector<TriGridIdx> &neighbor_idxs) {
        ...
        for (int n_i = 0 ; n_i < tmp_neighbors.size() ; n_i++) {
    ...
    void BreadthFirstTraversableGraphSearch(TriGridField<PointType>& tgf_in) {
        ...
        while (!searching_idx_queue.empty()){
            ...
            for (int i = 0; i < neighbor_idxs.size(); i++){
    ...
    TriGridCorner getMeanCorner(const TriGridCorner &corners_in){
        ...
        double sum_w = 0.0;
        for (int i = 0; i < corners_in.zs.size(); i++){
    ...
    void revertTraversableNodes(std::vector<std::vector<TriGridCorner>>& trigrid_corners_in,
        ...
        for (int r_i = 0; r_i < rows_; r_i++) {
        for (int c_i = 0; c_i < cols_; c_i++) {
        for (int s_i = 0; s_i < tgf_out[r_i][c_i].size(); s_i++) {
    ...
    void segmentTGFGround(const TriGridField<PointType>& tgf_in, 
        ...
        for (int s_i = 0; s_i < tgf_in[r_i][c_i].size(); s_i++) {
            ...
            if (tgf_in[r_i][c_i][s_i].node_type == GROUND) {
                segmentNodeGround(tgf_in[r_i][c_i][s_i], ptCloud_nodewise_ground_, ptCloud_nodewise_nonground_, ptCloud_nodewise_outliers_);
            } else {
    ...
    void publishTriGridFieldGraph() {
        ...
        for (int r_i = 0; r_i < rows_; r_i++){
        for (int c_i = 0; c_i < cols_; c_i++){
        for (int s_i = 0; s_i < trigrid_field_[r_i][c_i].size(); s_i++){
        ...
        for (int e_i = 0; e_i < trigrid_edges_.size(); e_i++){
    ...
    void publishTriGridCorners() {
        ...
        // for corners
        for (int r_i = 0; r_i < trigrid_corners_.size(); r_i++){
        for (int c_i = 0; c_i < trigrid_corners_[0].size(); c_i++){
            curr_corner = trigrid_corners_[r_i][c_i];
            if (curr_corner.zs.size() != curr_corner.weights.size()){
                ROS_WARN("ERROR in corners");
            }
            for (int i = 0; i < curr_corner.zs.size(); i++){
        ...
        // for centers
        for (int r_i = 0; r_i < trigrid_centers_.size(); r_i++){
        for (int c_i = 0; c_i < trigrid_centers_[0].size(); c_i++){
